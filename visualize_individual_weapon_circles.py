...
print(f"\n{'='*80}")
print(f"GENERATING INDIVIDUAL WEAPON RANGE CIRCLES")
print(f"{'='*80}\n")
...
for idx, weapon_row in df.iterrows():
    ...
    print(f"Processing: {gun_name}")
    
    # Determine best headshot combo (prioritize 1HS, then 2HS)
    # Test 1HS + body shots
    test_range_1hs = find_max_range(dmg_close, dmg_10m, dmg_75m, 1, 10, BASE_HS_MULT)
    
    # Test 2HS + body shots
    test_range_2hs = find_max_range(dmg_close, dmg_10m, dmg_75m, 2, 10, BASE_HS_MULT)
    
    # Determine which scenario to use (prefer 1HS if viable)
    if test_range_1hs >= 20:  # If 1HS can reach at least 20m, use it
        num_hs = 1
        title_suffix = "1 Incidental Headshot"
    else:
        num_hs = 2
        title_suffix = "2 Headshots"
    ...
    # Plot circles (order: Synth, HP, Base, then dot on top)
    if synth_range > 0:
        circle_synth = patches.Circle((0, 0), synth_range, color='purple', alpha=0.3, linewidth=2, 
                                      edgecolor='purple', label=f'Synthetic: {synth_range:.1f}m (+{synth_increase:.0f}%)')
        ax.add_patch(circle_synth)
    
    circle_hp = patches.Circle((0, 0), hp_range, color='orange', alpha=0.3, linewidth=2, 
                               edgecolor='orange', label=f'Hollow Point: {hp_range:.1f}m (+{hp_increase:.0f}%)')
    ax.add_patch(circle_hp)
    
    circle_base = patches.Circle((0, 0), base_range, color='gray', alpha=0.5, linewidth=2, 
                                 edgecolor='black', label=f'Base: {base_range:.1f}m')
    ax.add_patch(circle_base)
    
    # Player position (smaller dot on top)
    player_dot = patches.Circle((0, 0), 2, color='black', zorder=1000)
    ...
    # Title and labels
    ax.set_title(f'{gun_name} - Effective Range ({title_suffix})\n{weapon_class}', 
    ...
    # Save
    output_path = f'visualizations/INDIVIDUAL_WEAPONS/{gun_name}.png'
    plt.tight_layout()
